...
\mypar{Semantic annotation} Available techniques using semantics to analyze event logs \cite{} typically expect that each event can be associated with at least one \textit{action} and at least one \textit{object}. Building on the example introduced above, the action for event 1 would be ``\textit{receive}'' and the object would be ``\textit{order}''. While there are several techniques to derive actions and objects from labels \cite{}, we need to infer these components from the available UI attributes. To achieve this... \todo{To be finished.}
