...
However, such event classes are not readily available for UI logs. Rather, as shown in the example of \autoref{fig:example}, events are characterized by various attributes that jointly indicate the type of step that is being performed. For instance, in the form of  an application (e.g., Outlook), and further details, such as the element label (e.g., a particular e-mail) and its value (e.g., the message content). 
\todo{give an example of similar events with different actions}

 As a first step to prepare a UI log for process mining, we aim to classify each event in an UI log, allowing us to assign a label to events and thus to recognize which events correspond to the same kind of action. 
 
 how to classify:
event label is typically an action and object, where in this case the object can consist of e.g., the application and specific element label (e.g., what button/window in a given application), whereas the action can be 
 - tools for analysis of polluted and synonymous event labels, adapted to specific use case at hand (finding relevant attributes, role identification, some NLP, etc, can refer to this AI4BPM paper)
