...
\mypar{Semantic annotation - where} In traditional event logs, the application context in which an event has occurred is typically clear. That is, because the application itself records the occurrence of the event. In UI logs, the recording of the event is conducted independently of the underlying applications. Therefore, the application in which an event occurred must be inferred from the available UI attributes. While in many cases, the application attribute indicating the \textit{active application} is sufficient, this is, for instance, not the case if an event relates to a web browser. To be able to deal with problem, we introduce ... \todo{something awesome.}
