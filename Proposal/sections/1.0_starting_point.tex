...
A closer look a at the UI log from Figure \ref{fig:example} reveals that conducting process discovery on such a log is a complex task. Two main objectives \todo[do properly}
\begin{itemize}
\item Process recognition: from raw data to low level labled event log. that incvolves three challenges. ( 3 out of the 4 fundamental process mining requirements are not me by ui logs)
\item Process representation: from low level - event log to process model. 
\end{itemize}

Specifically, five main challenges need to be overcome: 
